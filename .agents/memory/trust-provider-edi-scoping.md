...
The config UI labels the provider field "Provider (label only — does not filter members)" so admins aren't misled. A golden-file format check (`scripts/dev/check-kaiser-edi-format.ts`) pins the 65-field/1120-byte Kaiser layout and overpunch encoding against an independent transcription of the PHP layout. It is no longer a registered validation — run it by hand until the provider-EDI conformance harness replaces it.
