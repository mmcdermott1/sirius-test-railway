 ...
 * create (1065) and this conversion back to back.
 */
async function up(): Promise<void> {
  ...
  if (oldType && newType) {
    throw new Error(
      "wc_stats has both a `day` and an `ymd` column; resolve manually before re-running migration 1066.",
    );
  }
  ...
  if (oldType) {
    ...
    if (bad > 0) {
      throw new Error(
        `Cannot convert wc_stats.day to date: ${bad} row(s) hold a value that is not a YYYY-MM-DD day; resolve manually before re-running migration 1066.`,
      );
    }
  ...
  if (await constraintExists("wc_stats", "wc_stats_service_type_day_uniq")) {
    if (await constraintExists("wc_stats", "wc_stats_service_type_ymd_uniq")) {
      throw new Error(
        "wc_stats carries both the old and the new uniqueness constraint; resolve manually before re-running migration 1066.",
      );
    }
    await db.execute(sql`
