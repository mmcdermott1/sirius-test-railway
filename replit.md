...
-   **Ledger System**: Manages financial transactions with comprehensive features:
    ...
    -   **Transactions**: Account-level transaction view with filtering (entity type, entity name, amount range, date range, description), sorting, pagination, and CSV export. Uses INNER JOIN on ledgerEa to ensure proper account scoping - orphaned entries (without EA records) cannot be displayed as account ownership is indeterminate without schema changes.
    -   **Entity-Account (EA)**: Links entities to accounts for proper transaction scoping.
