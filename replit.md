...
-   **Wizards**: Flexible workflow state management system for tracking multi-step processes (imports, bulk operations, etc.) with JSON data storage, type/status filtering, current step tracking, and full audit logging via Winston. The wizards table includes a `current_step` field that stores the step ID of the wizard's current position; if not set during creation, it defaults to the first step from the wizard type's steps() method.
    ...
    -   **Step Completion Validation**: Comprehensive validation system preventing advancement to next wizard step until current step is complete. Frontend step controller registry (`client/src/components/wizards/steps/registry.ts`) provides evaluators for Upload (requires valid file) and Map (requires all required fields mapped) steps. WizardNavigator disables Next button with tooltip and alert banner when step is incomplete. Backend mirrors validation in `/steps/next` endpoint to prevent API bypass, returning 400 error if current step requirements aren't met. Step completion state tracked via `stepStatus` field in WizardData schema.
