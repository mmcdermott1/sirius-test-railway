...
-   **Work Status History Auto-Sync**: The `worker_wsh` table tracks historical work status entries with automatic synchronization to `workers.ws_id`. Each time a work status history entry is created, updated, or deleted, the worker's current work status is automatically updated to reflect the most recent history entry (ordered by date DESC, then createdAt DESC). The `workerWsh` table includes a `createdAt` timestamp field to ensure proper temporal ordering when multiple entries exist on the same date. The sync logic uses robust ordering with NULLS LAST and id DESC fallback for deterministic results.
